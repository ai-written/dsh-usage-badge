...
`
    ...
    function buildSeries(snapshot, range, provider) {
      ...
      if (range === '12m') {
        ...
        for (const day of snapshot.days) {
          ...
          months.get(key).push(entryTotals(day, provider))
        }
        const keys = [...months.keys()].sort().slice(-12)
        return {
          points: keys.map((key) => point(key.slice(2), key, sumTotals(months.get(key)))),
          marks: false,
        }
      }

      const count = range === '7d' ? 7 : 30
      const slice = snapshot.days.slice(0, count).reverse()
      return {
        points: slice.map((day) =>
          point(
            day.date.slice(5),
            `${day.date}${day.dayName ? `  ${day.dayName}` : ''}`,
    ...
    function ConfigPanel({ snapshot, reload, initialConfig = null, initialOfficial = null }) {
      ...
      const [official, setOfficial] = React.useState(initialOfficial)
      const [officialState, setOfficialState] = React.useState(initialOfficial ? 'ready' : 'loading')
      const [applyState, setApplyState] = React.useState('idle')
      const [config, setConfig] = React.useState(initialConfig)

      const loadOfficial = React.useCallback(
        async (which) => {
    ...
    function UsageDialog({ snapshot, onClose, reload, initialTab = 'usage', initialConfig = null, initialOfficial = null }) {
      ...
      return h(
        ...
        h(
          ...
          // Only the active panel is mounted, so returning to 单价配置 re-fetches
          // the published list — which is the behavior that was asked for.
          tab === 'usage'
            ? h(UsagePanel, { snapshot })
     ...
     *   initialTab?: string, initialConfig?: object|null, initialOfficial?: object|null}} props
     */
    function UsageBadge({
      wide = true,
      ...
      return h(
        ...
        open && snapshot
          ? h(UsageDialog, { snapshot, onClose: () => setOpen(false), reload: load, initialTab, initialConfig, initialOfficial })
          : null,
      )
    }
