 ...
 *
 * Two rules were added on top of that port, both opt-out and both documented in
 * `docs/design-notes.md`: a trailing `*` on a model part makes a family row, and — unless
 * `modelPrefixFallback` is off — an ordinary row also prices its own separator-suffixed
 * variants (`deepseek-flash` covers `deepseek-flash-preview`). `priceUnmatchedModels` can
 * also be switched off, which makes a model no row matches cost nothing rather than being
 * priced by the `default` row.
 *
...
/**
 * The rates a table with no `default` row of its own falls back to, plus the exchange rate the
 * shell used. **Never written to disk** — nothing in this module (or the plugin) persists it, it
 * is only the in-memory fallback so a fresh install shows an estimate instead of zeroes. It is
 * served to the panel as `template` so those rates can be labelled as what they are, and
 * 「应用官方价格」 replaces them with the published list's first line.
 */
...
export function normalizePricing(raw) {
  ...
  return {
    ...
    // Implicit prefix fallback: an authored row also prices the suffixed variants of
    // its own name, so the `deepseek-flash` row the official list writes covers
    // `deepseek-flash-preview` with nothing hand-written. On unless switched off, and
    // only an explicit `false` switches it off.
    modelPrefixFallback: user.modelPrefixFallback !== false,
    // Whether a model that no row matches is priced by the `default` row at all. On by
    // default — pricing everything is what that row is for — and switched off it means
    // "we do not know what this cost": the request contributes tokens and requests but no
    // amount, and the snapshot reports which routes those are rather than a silent ¥0.
    priceUnmatchedModels: user.priceUnmatchedModels !== false,
 ...
 * every provider.
 *
 * These are the *exact* keys a route can resolve to. Rows that end a model part with
 * `*` are family rules and are appended after these by {@link rowsForRoute}, because
 * an exact row always outranks a family one.
 *
...
/**
 * The prefix a family row matches on, or null when the key is not a family row.
 *
 * A trailing `*` on the model part means "every model whose id starts with this", so
 * the row `deepseek-flash-*` prices `deepseek-flash-preview` and any dated or
 * experimental variant of the family without a row per name.
 *
 * The prefix must be non-empty, and that is exactly what keeps a bare `*` inert:
 * `provider|*` already means "any model" through the exact candidate list, and a lone
 * `*` key has never matched anything. The star is not a separator either, so
 * `deepseek-flash-*` does not cover the model `deepseek-flash` itself — write
 * `deepseek-flash*` when the family includes the unsuffixed name.
 */
function familyPrefix(pattern) {
  const text = String(pattern)
  return text.endsWith('*') && text.length > 1 ? text.slice(0, -1) : null
}

/**
 * What must follow a prefix for the *implicit* fallback to accept it: a separator.
 *
 * Without this, `gpt-4` would swallow `gpt-4o` — a different model with a different
 * price — and `deepseek-flash` would swallow `deepseek-flashlight`. With it, the
 * fallback only ever reaches `deepseek-flash-preview`, `deepseek.v3`, `qwen:72b`
 * shapes. An explicit `deepseek-flash*` row skips this test: writing the star is how
 * you say "yes, the glued-together names too".
 */
const PREFIX_BOUNDARY = /^[-_.:/@]/

/**
 * The prefix rows that apply to one route, most specific first.
 *
 * Two kinds end up here, and they share one ranking:
 *
 *   - explicit family rows, a model part ending in `*` (`deepseek-flash-*`);
 *   - implicit ones, when `modelPrefixFallback` is on: an ordinary row also prices the
 *     suffixed variants of its own name, which is what lets the row the official price
 *     list writes — `deepseek-flash` — cover `deepseek-flash-preview` with nothing
 *     hand-written at all.
 *
 * Ranked by the shape they take — pure model, `provider|model`, `*|model`, the same
 * order the exact keys resolve in — then by prefix length (the narrow `deepseek-flash-*`
 * outranks the broad `deepseek-*`), and explicit wins a tie against implicit. A key that
 * names a different provider, or has more than one `|`, belongs to no shape and is
 * ignored, exactly as it was when it could never match.
 */
function familyRows(rows, provider, model, fallback) {
  if (!model) return []
  const routeProvider = String(provider ?? '').toLowerCase()
  const routeModel = String(model).toLowerCase()
  const found = []
  for (const key of Object.keys(rows)) {
    const parts = String(key).split('|')
    if (parts.length > 2) continue
    const modelPart = String(parts.length === 2 ? parts[1] : parts[0])
    const prefix = familyPrefix(modelPart)
    let length
    if (prefix !== null) {
      if (!routeModel.startsWith(prefix.toLowerCase())) continue
      length = prefix.length
    } else if (fallback) {
      const name = modelPart.toLowerCase()
      // The exact key has already had its turn by the time this runs, so an implicit
      // row is always a *strict* prefix plus a separator.
      if (!name || !routeModel.startsWith(name) || !PREFIX_BOUNDARY.test(routeModel.slice(name.length))) continue
      length = name.length
    } else {
      continue
    }
    let shape
    if (parts.length === 1) shape = 0
    else if (parts[0] === '*') shape = 2
    else if (parts[0].toLowerCase() === routeProvider) shape = 1
    else continue
    // A row can be `null` in the file — a hand edit, or a delete that left the key behind. The
    // exact-key path has always skipped those; skipping them here too is what keeps a single such
    // row from throwing out of every price lookup and turning `/summary` into a permanent 500.
    const row = rows[key]
    if (!row) continue
    found.push({ shape, length, explicit: prefix === null ? 0 : 1, row })
  }
  return found
    .sort((a, b) => a.shape - b.shape || b.length - a.length || b.explicit - a.explicit)
    .map((entry) => entry.row)
}

/**
 * The override rows that price one route, highest priority first.
 *
 * Exact keys come first, in the order {@link matchKeys} establishes, so a table that
 * writes no family row resolves exactly as it did before families existed. Prefix rows
 * follow — same shape order, longest prefix first — which is what lets a family rule
 * cover the models nobody wrote a row for without ever outranking one that somebody did.
 *
 * Memoized per override set and route: this is consulted once per price field of every
 * request record while a day is folded. The entry is only reusable while the override *map*
 * is the same object — adding or removing a key in place would go unnoticed, so callers must
 * treat a document as immutable (every path here re-parses the file instead of editing it).
 * The fallback switch is part of the cache key because two normalized documents can share one
 * override map and differ only by that flag — which is exactly what toggling the switch does
 * when a caller re-normalizes without re-reading.
 */
const routeRows = new WeakMap()

function rowsForRoute(pricing, provider, model) {
  const rows = pricing.overrides
  if (!rows) return []
  const fallback = pricing.modelPrefixFallback !== false
  let byRoute = routeRows.get(rows)
  if (!byRoute) {
    byRoute = new Map()
    routeRows.set(rows, byRoute)
  }
  const routeKey = `${fallback ? 'p' : 'e'}\u0000${provider ?? ''}\u0000${model ?? ''}`
  const cached = byRoute.get(routeKey)
  ...
  const all = resolved.concat(familyRows(rows, provider, model, fallback))
  byRoute.set(routeKey, all)
  return all
}
...
export function resolvePrice(pricing, provider, model) {
  const out = {}
  const rows = rowsForRoute(pricing, provider, model)
...
export function modelTimeOfUse(pricing, provider, model) {
  for (const row of rowsForRoute(pricing, provider, model)) {
    if (row.timeOfUse) return row.timeOfUse
  }
...
export function modelMultiplier(pricing, provider, model) {
  for (const row of rowsForRoute(pricing, provider, model)) {
    if (row.multiplier != null) return Number(row.multiplier) || 1
  }
...
export function modelContextMultiplier(pricing, provider, model) {
  for (const row of rowsForRoute(pricing, provider, model)) {
    if (row.contextMultiplier != null) return row.contextMultiplier
  }
...
/**
 * Whether a route has no price at all.
 *
 * True only when `priceUnmatchedModels` is switched off *and* no override row applies to
 * the route. A row that matches counts even when it defines nothing but a multiplier:
 * "matched" means the table has something to say about this model, and the `default` row
 * supplies whatever the row leaves out.
 *
 * @param {object} pricing - the normalized pricing document.
 * @param {string|null} provider - route provider.
 * @param {string|null} model - route model.
 */
export function isUnpriced(pricing, provider, model) {
  if (pricing.priceUnmatchedModels !== false) return false
  return rowsForRoute(pricing, provider, model).length === 0
}
...
export function requestCost(tokens, provider, model, hour, pricing, day) {
  ...
  if (isUnpriced(pricing, provider, model)) return 0
 ...
 * rule — so the holiday check comes before every `days` form. A rule can decline that
 * calendar with `honorHolidays: false`, which is what a gateway that keeps charging its
 * own peak windows on a Chinese public holiday needs; the day is then judged by `days`
 * like any other. Weekends need no such flag: they are already per rule through `days`
 * (`"all"`, `"weekend"`, `[6,7]`).
 *
 * @param {number} weekday - ISO weekday (1=Mon … 7=Sun).
...
export function multiplierFor(weekday, hour, tou, dayClass = 'normal') {
  ...
  if (dayClass === 'holiday' && tou.honorHolidays !== false) return 1
