 ...
 * are not lost — including a `default` row, which is only ever *filled in* when the
 * document has none (see below).
 *
 * @param {object} current - the raw user pricing document.
 * @param {object} parsed - a {@link parseOfficialPricing} result.
 * @returns {{document:object, applied:{models:string[], aliases:string[], peakRanges:number[][], defaultFrom:string|null}}}
 */
export function applyOfficialPricing(current, parsed) {
  ...
  // A table with no `default` row of its own is priced by the built-in template, and those
  // rates are nobody's choice. Fill it from the published list's **first** line instead: the
  // page leads with its cheapest model, so a model the table does not name is guessed low
  // rather than invented. A `default` row the user already wrote is still left alone — this
  // only replaces the absence of one.
  let defaultFrom = null
  if (!next.default && parsed.models.length > 0) {
    next.default = rowFor(parsed.models[0])
    defaultFrom = parsed.models[0].model
  }
  next.timeOfUse = {
    ...(next.timeOfUse ?? {}),
    enabled: true,
  ...
  return {
    document: next,
    applied: { models: applied, aliases: aliasNames, peakRanges: parsed.policy.peakRangesLocal, defaultFrom },
  }
}
